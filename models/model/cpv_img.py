    ...
    def collate(self):
        def collate_fn(batch):
            ...
            for idx in range(batch_size):
                ...
                highs.append(batch[idx][0]) # -> seq_len x 512 x 7 x 7
...
class Module(Base):
    ...
    def forward(self, highs, contexts, targets, highs_lens, contexts_lens, targets_lens, mask):
        ...
        '''
        ### INPUT ###
        # Highs ->  B x M x 512 x 7 x 7 (M = Highs Seq Len)
        ...
        ### HIGHS ###
        # Reshaping:
        flat_highs = highs.reshape(batch_size * highs_len, resnet_size[0], resnet_size[1], resnet_size[2]) # -> B * M x 512 x 7 x 7
        # Dimension Reduction:
        ...
        ### TARGETS ###
        # Reshaping:
        flat_targets = targets.reshape(batch_size * targets_len, resnet_size[0], resnet_size[1], resnet_size[2]) # -> B * T x 512 x 7 x 7
        # Dimension Reduction:
